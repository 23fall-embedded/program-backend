...
public class DataController {
    ...
    AmqpClient amqpClient = new AmqpClient(this);

    @GetMapping("/getData")
    public Map<String, String[]> getData() {
        while(amqpClient.getStackContent().isEmpty() || paramsService.countParamsRows() == 0) {
            try {
                Thread.sleep(1000);
        ...
        Content latestContent = 
            amqpClient.getStackContent().isEmpty() ? 
                paramsService.getAndTurnParamsLicenseIntoContent() : amqpClient.getStackContent().peek();
